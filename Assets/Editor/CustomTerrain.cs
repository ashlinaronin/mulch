...
public class CustomTerrain : MonoBehaviour {
	...
	public Color cloudLiningColor = Color.grey;
	public float cloudMinSpeed = 0.2f;
	public float cloudMaxSpeed = 0.5f;
	public float cloudDistanceTraveled = 500.0f;
	...
	public void Voronoi()
	{
		...
		{
			float x = UnityEngine.Random.Range(0, terrainData.heightmapResolution);
			float y = UnityEngine.Random.Range(0, terrainData.heightmapResolution);
		...
		float maxDistance = Vector2.Distance(new Vector2Int(0, 0), new Vector2Int(terrainData.heightmapResolution, terrainData.heightmapResolution));
	...
	public void DrawShoreline()
	{
		...
		for (int x = 0; x < terrainData.heightmapResolution; x++)
		{
			for (int y = 0; y < terrainData.heightmapResolution; y++)
			{
				...
				List<Vector2> neighbors = GetNeighbors(thisLocation, terrainData.heightmapResolution, terrainData.heightmapResolution);
				...
				foreach (Vector2 neighbor in neighbors)
				{
					if (heightMap[x, y] < waterHeight && heightMap[(int)neighbor.x, (int)neighbor.y] > waterHeight)
					{
						...
						go.transform.LookAt(
							new Vector3(
								neighbor.y / (float)terrainData.heightmapResolution * terrainData.size.z,
								waterHeight * terrainData.size.y,
								neighbor.x / (float)terrainData.heightmapResolution * terrainData.size.x
							)
						);
	...
	void Canyon()
	{
		...
		while (
			 ...
			 cy >= 0 && cy < terrainData.heightmapResolution
		)
		{
